...
module.exports = {
    ...
    getTeamRoleSlot: async function (id, role) {
        try {
            const [rows] = await dbconnection.query(`SELECT * FROM teams WHERE disc_role_id = ?`, id);
            if (role.trim() === 'top') { return rows[0].top; }
            if (role.trim() === 'jungle') { return rows[0].jungle; }
            if (role.trim() === 'mid') { return rows[0].mid; }
            if (role.trim() === 'bot') { return rows[0].bot; }
            if (role.trim() === 'support') { return rows[0].support; }
        } catch (error) {
            console.error('Error fetching teams from database:', error);
            throw error;
        }
